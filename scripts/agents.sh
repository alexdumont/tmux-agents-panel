...
# Get agent status: Working / Idle / Waiting
get_agent_status() {
    local pid="$1"
    ...
    cpu=${cpu%.*}  # integer part

    # Check for active child processes (bash, node sub-commands, etc.)
    local children
    children=$(pgrep -P "$pid" 2>/dev/null | wc -l)

    if [[ "$children" -gt 0 ]]; then
        echo "working"
    elif [[ -n "$cpu" && "$cpu" -gt 5 ]]; then
        echo "thinking"
    else
        echo "idle"
    fi
...
# Get the tmux window/pane hosting this PID (best-effort)
get_tmux_location() {
    local pid="$1"

    # Walk up the process tree to find a pane
    local cur="$pid"
    for _ in $(seq 1 10); do
        ...
        # Check if any tmux pane has this ppid as its tty process
        local pane_info
        pane_info=$(tmux list-panes -a -F '#{pane_pid} #{window_index}:#{pane_index} #{window_name}' 2>/dev/null | awk -v p="$ppid" '$1==p{print $2" "$3}')
        if [[ -n "$pane_info" ]]; then
            echo "$pane_info"
            return
        fi
